...
export async function loadCredentials(): Promise<{ baseUrl: string; token: string }> {
...
export async function saveCredentials(baseUrl: string, token: string): Promise<void> {
  ...
  _token = token;
  await Promise.all([
    AsyncStorage.setItem(BASE_URL_KEY, _baseUrl),
...
export const api = {
  ...
  getFileDiff: (sessionId: string, filePath: string, maxLines = 2000) =>
    req<DiffResult>(
      'GET',
      `/sessions/${sessionId}/files/${encodeURIComponent(filePath)}/diff?maxLines=${maxLines}`,
    ),
  ...
  getRepoFile: (sessionId: string, repoPath: string, maxBytes = 524288) =>
    req<RepoFileContent>(
      'GET',
      `/sessions/${sessionId}/repo/file?path=${encodeURIComponent(repoPath)}&maxBytes=${maxBytes}`,
    ),
...
export function buildWsUrl(sessionId: string, channel: 'agent' | 'shell' = 'agent'): string {
  ...
  return `${wsBase}${path}?token=${encodeURIComponent(_token)}`;
}
